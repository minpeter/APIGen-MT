...
"""
    ...
    def _replay_state(
        self,
        trajectory: List[TrajectoryStep],
        baseline_state: Optional[Dict[str, Dict[str, Any]]] = None,
    ) -> None:
        """Restore API state after a failed state-verification attempt.

        Prefer restoring a snapshot captured before the failed tool call
        (or the datapoint initial snapshot). Never draws a new random config
        on rollback — force_new is only for starting a fresh datapoint.
        """
        if baseline_state is not None:
            self.tool_manager.restore_api_state(baseline_state)
            return

        # Fallback: re-init with CACHED config (force_new=False) then replay
        ...
        if "message_api" in state and "current_user" not in state.get("message_api", {}):
            for step in trajectory:
                for tc in step.tool_calls:
                    if tc.tool_name == "message_login" and "user_id" in tc.arguments:
                        self.tool_manager.python_tool_instances["message_api"].current_user = (
                            tc.arguments["user_id"]
                        )
                        return
