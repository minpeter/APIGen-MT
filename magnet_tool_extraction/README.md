...
Large `bfcl_v3_*.jsonl` artifacts are **gitignored**. Generate them with the
extraction scripts in this directory (or download from your data store), then
point `--tool-pool` / `--invocation-examples` at those paths.
