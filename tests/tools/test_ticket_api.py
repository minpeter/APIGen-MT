...
def test_close_ticket_normal(ticket_api: TicketAPI):
...
def test_close_ticket_not_found(ticket_api: TicketAPI):
    result = ticket_api.close_ticket(ticket_id=987654)
    assert "not found" in result.get("status", "").lower()


def test_close_ticket_already_closed(ticket_api: TicketAPI):
    _ = ticket_api.close_ticket(ticket_id=123456)
    result = ticket_api.close_ticket(ticket_id=123456)
    assert result.get("status") == "Ticket closed successfully"


def test_create_ticket_normal(ticket_api: TicketAPI):
...
def test_create_ticket_defaults(ticket_api: TicketAPI):
...
def test_create_ticket_clamp_priority_high(ticket_api: TicketAPI):
    result = ticket_api.create_ticket(title="Bad Priority", priority=10)
    assert result.get("priority") == 5


def test_create_ticket_clamp_priority_low(ticket_api: TicketAPI):
    result = ticket_api.create_ticket(title="Low Priority", priority=-1)
    assert result.get("priority") == 1


def test_edit_ticket_normal(ticket_api: TicketAPI):
...
def test_edit_ticket_not_found(ticket_api: TicketAPI):
    result = ticket_api.edit_ticket(ticket_id=0, updates={"status": "Urgent"})
    assert "not found" in result.get("status", "").lower()


def test_edit_ticket_invalid_field(ticket_api: TicketAPI):
    result = ticket_api.edit_ticket(ticket_id=123456, updates={"nonexistent_field": "value"})
    assert result.get("status") == "Ticket updated successfully"


def test_get_ticket_normal(ticket_api: TicketAPI):
...
def test_get_ticket_not_found(ticket_api: TicketAPI):
    result = ticket_api.get_ticket(ticket_id=987654)
    assert result.get("status") == "Not Found"
    assert result.get("id") == 0


def test_get_ticket_invalid_id(ticket_api: TicketAPI):
    result = ticket_api.get_ticket(ticket_id=-1)
    assert result.get("status") == "Not Found"
    assert result.get("id") == 0


def test_get_user_tickets_all(ticket_api: TicketAPI):
    result = ticket_api.get_user_tickets()
    assert result.get("id") is not None
    assert result.get("created_by") == "tech_guru"


def test_get_user_tickets_by_status(ticket_api: TicketAPI):
    result = ticket_api.get_user_tickets(status="Open")
    assert result.get("status") == "Open"
    assert result.get("created_by") == "tech_guru"


def test_get_user_tickets_empty_status(ticket_api: TicketAPI):
    result = ticket_api.get_user_tickets(status="Closed")
    assert result.get("id") == 0
    assert result.get("title") == ""


def test_resolve_ticket_normal(ticket_api: TicketAPI):
...
def test_resolve_ticket_not_found(ticket_api: TicketAPI):
    result = ticket_api.resolve_ticket(ticket_id=7423, resolution="")
    assert "not found" in result.get("status", "").lower()


def test_resolve_ticket_already_resolved(ticket_api: TicketAPI):
    _ = ticket_api.resolve_ticket(
        ticket_id=123456, resolution="First resolution"
    )
    result = ticket_api.resolve_ticket(ticket_id=123456, resolution="Second resolution")
    assert result.get("status") == "Ticket resolved successfully"


def test_ticket_login_normal(ticket_api: TicketAPI):
    result = ticket_api.ticket_login(username="tech_guru", password="testpass")
    assert result.get("success") is True
    assert ticket_api.current_user == "tech_guru"


def test_ticket_login_invalid_credentials(ticket_api: TicketAPI):
    result = ticket_api.ticket_login(username="user123", password="12345")
    assert result.get("success") is True


def test_ticket_login_missing_fields(ticket_api: TicketAPI):
...
def test_create_ticket_requires_authentication():
    ...
    result = ticket_api.create_ticket(title="Unauthorized")
    assert result["status"] == "User not authenticated"


def test_ticket_login_rejects_configured_credentials():
    ticket_api = TicketAPI({"username": "agent", "password": "secret"})
    assert ticket_api.ticket_login("agent", "wrong")["success"] is False
    assert ticket_api.ticket_login("agent", "secret")["success"] is True
